...
from .models import Evento, Certificados
...
def participantes_evento(request, id):
 ...
 # Função que gerar e exporta em CSV   
...
def certificados_evento(request, id):
    ...
    if request.method == "GET":
        qtd_certificados = evento.participantes.all().count() - Certificados.objects.filter(evento=evento).count()
...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        certificado_gerado = Certificados(
...
def procurar_certificado(request, id):
    ...
    certificado = Certificados.objects.filter(evento=evento).filter(participante__email=email).first()
